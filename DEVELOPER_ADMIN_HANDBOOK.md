...
Checks phone number is OTP verified
Hashes password
...
## 6A. Student Phone OTP Verification

DataGrip can store and show phone/OTP data, but DataGrip cannot send OTPs.

The backend does OTP generation:

```text
Student enters phone number
Student clicks Send OTP
Frontend calls POST /api/request-otp
Backend generates 6 digit OTP
Backend stores OTP in otp_verifications
Backend sends OTP by SMS when SMS_PROVIDER is configured
Student enters OTP
Frontend calls POST /api/verify-otp
...
Local college-project testing mode:

```text
OTP can be generated by backend and shown on screen only when running locally or when SMS_DEMO_MODE=true.
```

Monitor verified students:

```sql
SELECT id, name, roll_number, phone_number, phone_verified, created_at
FROM users
WHERE role = 'student'
ORDER BY id DESC;
```
...
```sql
SELECT phone_number, otp_code, verified, expires_at, created_at
...
## 13A. Why New Deployed Users May Not Appear In Local DataGrip

This is very important.

Your local DataGrip opens:

```text
/Users/tirumalarajavardhan/Downloads/annamacharya-university-portal/annamacharya_portal.sqlite3
```

Your free Render deployment uses:

```text
/tmp/annamacharya_portal.sqlite3
```

These are two different database files.

So:

```text
User registered on localhost -> visible in local DataGrip
User registered on Render URL -> not visible in local DataGrip
```

Because free Render uses temporary server storage, deployed data can also reset after redeploy/restart.

To see deployed users, you must inspect the deployed server database from Render shell/logs, or move to a permanent cloud database later.
