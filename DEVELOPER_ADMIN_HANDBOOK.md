...
Demo mode returns OTP to browser
Student enters OTP
...
For a real SMS OTP later, connect an SMS provider in `request_otp()`:
...
Current free college-project mode:

```text
OTP is generated by backend and shown on screen for demo.
```
